# CSUMB Certificate Track Capstone Dashboard

Target completion date: **August 7, 2026**

Estimated capstone work target: **160-200 hours**

## Quick Links

| Area            | Notes                                                                                                                                                                                          |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Planning        | [[01 Timeline]] · [[02 CSUMB Milestones]]                                                                                                                                                      |
| Weekly tracking | [[03 Weekly Work Log]] · [[Templates/Weekly Log Template]]                                                                                                                                     |
| Certificates    | [[Certificates/AZ-900 Azure Fundamentals]] · [[Certificates/AZ-104 Azure Administrator]] · [[Certificates/MD-102 Endpoint Administrator]] · [[Certificates/Copilot and AI Admin Fundamentals]] |
| Projects        | [[Projects/Azure Labs]] · [[Projects/PowerShell Automation]] · [[Projects/Endpoint Management]] · [[Projects/Final Applied Project]]                                                           |
| Evidence        | [[Evidence/Screenshot Log]] · [[Evidence/Certificate Completion Log]] · [[Evidence/Portfolio Evidence Tracker]]                                                                                |
| CSUMB updates   | [[CSUMB Updates/Week 8 ILP Update Draft]] · [[CSUMB Updates/Week 14 Artifact Submission Draft]] · [[CSUMB Updates/Week 16 Final Presentation Notes]]                                           |

## Current Focus

- [ ] Confirm certificate priority for this week.
- [ ] Choose labs or modules to complete.
- [ ] Capture screenshots or notes as evidence.
- [ ] Update [[03 Weekly Work Log]].
- [ ] Add portfolio-ready artifacts to [[Evidence/Portfolio Evidence Tracker]].

## Hour Summary

| Category | Target Hours | Completed Hours | Notes |
| --- | ---: | ---: | --- |
| AZ-900 Azure Fundamentals | TBD | 13.58 | [[Certificates/AZ-900 Azure Fundamentals]] |
| AZ-104 Azure Administrator | TBD | 0 | [[Certificates/AZ-104 Azure Administrator]] |
| MD-102 Endpoint Administrator | TBD | 0 | [[Certificates/MD-102 Endpoint Administrator]] |
| Copilot / AI admin fundamentals | TBD | 0 | [[Certificates/Copilot and AI Admin Fundamentals]] |
| PowerShell automation projects | TBD | 0 | [[Projects/PowerShell Automation]] |
| Portfolio and CSUMB updates | TBD | 0 | [[02 CSUMB Milestones]] |
| **Total** | **160-200** | **13.58** |  |

## Evidence Checklist

- [ ] Training module completion records
- [ ] Lab screenshots
- [ ] Configuration notes
- [ ] PowerShell scripts or GitHub commits
- [ ] Portfolio writeups
- [ ] Certificate exam results or completion confirmations
- [ ] Final presentation artifacts

## Next Review

Use [[Templates/Weekly Log Template]] to add the next entry to [[03 Weekly Work Log]].
